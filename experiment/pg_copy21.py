...
def change_pg_statistics(connection, cursor, epsilon=0.1):
    ...
    # cr = changed rows
    cr = insert_vals_into_rtc(noisy_vals3, rtc, 21)
    print("2.5")
...
def main():
    ...
    try:
        ...
        s = time.time()
        eps_lst = [0.0005, 0.0025, 0.0075, 0.01, 0.0125, 0.025, 0.0375, 0.05, 0.075, 0.1]
        for eps in eps_lst:
            change_pg_statistics(connection, cursor, eps)
